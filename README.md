...
Цель работы — изучить назначение и принципы работы синтаксического анализатора в структуре компилятора, спроектировать грамматику, построить схему метода анализа грамматики и выполнить программную реализацию парсера с нейтрализацией синтаксических ошибок методом Айронса, а также интегрировать разработанный модуль в ранее созданный графический интерфейс языкового процессора.
...
Синтаксическая конструкция описывается контекстно-свободной грамматикой:
...
```text
Vt = { val, identifier, =, listOf, (, ), ,, ;, string, char, int, double, true, false, +, - }
```

### Нетерминальные символы

```text
Vn = { Z, <declaration_list>, <declaration>, <elements_opt>, <elements>, <element>, <number>, <sign> }
```
...
Это объясняется тем, что в левой части каждого правила продукции находится ровно один нетерминальный символ.

## 7. Метод анализа

Для синтаксического анализа выбран метод **рекурсивного спуска**.

Для каждого нетерминального символа грамматики в программе реализован отдельный метод:

- `ParseDeclaration()` — разбор объявления списка;
- `ParseElementsOpt()` — разбор пустого или непустого списка элементов;
- `ParseElements()` — разбор последовательности элементов списка;
- `ParseElement()` — разбор одного элемента списка;
- `ParseNumberLiteral()` — разбор числового литерала;
- `ParseSignedNumber()` — разбор числа со знаком;
- `ParseSign()` — разбор знака `+` или `-`.

Рекурсивный спуск был выбран по следующим причинам:

- грамматика варианта имеет простую иерархическую структуру;
- метод легко реализовать на языке C#;
- удобно встроить диагностику ошибок прямо в методы разбора;
- удобно реализовать нейтрализацию ошибок методом Айронса.

### Граф автоматной грамматики

Ниже приведён граф автоматной грамматики для синтаксической конструкции объявления списка с инициализацией на языке Kotlin.

![Граф автоматной грамматики](images/lab3/11-parser-graph.png)

На рисунке представлен граф автоматной грамматики для синтаксической конструкции объявления списка с инициализацией на языке Kotlin. Переходы по дугам соответствуют терминальным символам грамматики. После чтения ключевого слова `val`, идентификатора, оператора присваивания и лексемы `listOf` анализатор переходит к разбору содержимого списка. Допускается как пустой список, так и непустой список, состоящий из строковых, символьных, логических и числовых литералов, включая числа со знаком.
...
Парсер работает поверх результата лексического анализа. После нажатия кнопки **«Пуск»** сначала запускается лексический анализатор. Если лексические ошибки отсутствуют, результат передаётся в синтаксический анализатор.
...
- реализована нейтрализация ошибок методом Айронса;
