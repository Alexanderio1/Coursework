...
- грамматика варианта имеет иерархическую структуру
- рекурсивный спуск удобно реализовать на языке C#
- метод позволяет выполнять проверку синтаксиса последовательно и понятно
- диагностику ошибок удобно встраивать непосредственно в процедуры разбора
- нейтрализацию синтаксических ошибок методом Айронса удобно совмещать с таким способом анализа

В программной реализации разбор организован в виде набора процедур, соответствующих основным нетерминалам грамматики. Ключевую роль выполняют следующие методы:
...
Дополнительно в анализаторе реализованы проверки числовых литералов, знаков `+` и `-`, ожидаемых разделителей и завершающих символов.
...
На рисунке показана схема рекурсивного спуска для конструкции объявления списка с инициализацией на языке Kotlin. После распознавания ключевого слова `val`, идентификатора, оператора присваивания и лексемы `listOf` анализатор переходит к разбору содержимого списка. Далее допускается либо пустой список, либо непустой список элементов. Каждый элемент может быть строковым, символьным, логическим или числовым литералом. После разбора очередного элемента выполняется проверка: требуется ли считывание следующего элемента через запятую или список завершается символом `)`.
